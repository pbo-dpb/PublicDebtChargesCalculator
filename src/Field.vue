<template>
    <div class="flex flex-col">
        ...
        <input :id="uuid" :class="classes" type="number" step="1" :value="modelValue"
...
export default {
    ...
    computed: {
        classes() {
            let classes = ['w-full', 'text-right', 'px-1', 'lg:px-2', 'py-1', 'border-2', 'print:border-0', 'font-semibold', 'rounded', 'text-xs', 'lg:text-base'];
            ...
            if (this.readonly !== undefined && this.readonly !== false) {
                ...
                if (this.isStatic) {
                    classes = classes.concat(['bg-gray-100', 'text-gray-600', 'dark:text-gray-400'])
                } else {
                    classes = classes.concat(['text-gray-800', 'dark:text-gray-200'])
                }
            } else {
